...
## Purpose
- **Browse** a local music library with recursive folder scanning
...
- **Smart Features**: Automatically merge split releases, clean artist names
...
- **Purpose**: Scan music directory recursively
- **Key Methods**:
  ...
  - `listTracks()`: Get all music files in a folder with metadata
...
- `GET /api/scan?action=list_files&path={path}`: List albums in path
...
### UI Components
1. **Navbar**: Breadcrumbs, theme toggle
2. **Album Grid**: Card-based layout with lazy-loaded covers
3. **Inline Detail View**: 
   - Left column: Local tracks table
   - Right column: Discogs search & comparison
4. **Smart View Toggle**: Merges related split releases
...
### Installation
1. Clone repository
2. Run `composer install` in `ci4/` directory
3. Copy `ci4/.env.example` to `ci4/.env`
4. Configure `MUSIC_ROOT_PATH` and Discogs API keys
5. Ensure PHP 8.1+ with GD extension enabled
...
### Performance
- **Recursive Scan**: ~1 second for 800 albums
...
2. Frontend calls `/api/scan?action=list_files&path=` 
3. `FileScanner::getAlbums()` recursively scans, returns album list with `has_cover` flags
4. Browser lazy-loads covers via `/api/scan?action=get_cover&path={path}`
5. User clicks album → `/api/scan?action=list_tracks&path={path}`
6. User clicks "Smart Search" → `/api/search?artist=X&album=Y&folder=Z`
7. User selects release → `/api/search?action=get_release&id={id}`
8. User clicks "Apply Tags" → `POST /api/tag` with file array
9. `TagManager::writeTags()` writes tags with preservation logic
